from dataprocess.Augmentation.images_masks_3dtransform import ImageDataGenerator3D
import pandas as pd
import numpy as np
...
class DataAug3D(object):
    ...
    def __ImageMaskTranform(self, images_path, index, number, maxvalue=255):
        ...
        for batchx, batchy in self.__datagen.flow(srcimage, srcmask):
            ...
            npy_path1 = self.aug_path + 'Image/' + str(self.aug_label)+str(index) + '_' + str(i) + ".npy"
            batch1 = batch1.reshape([srcimages.shape[0], srcimages.shape[1], srcimages.shape[2]])
            np.save(npy_path1, batch1)

            npy_path2 = self.aug_path + 'Mask/' + str(self.aug_label) + str(index) + '_' + str(i) + ".npy"
    ...
    def DataAugmentation(self, filepathX, number=100, aug_path=None, aug_label=None):
        ...
        for index in range(data.shape[0]):
            ...
            self.__ImageMaskTranform(images_path, index, number, self.aug_label)
